...
class UserServiceTest{
 ...
 @Test void create(){
    UserDTO d=new UserDTO("Ali","a@x.com","CITIZEN");when(repo.save(any())).thenAnswer(i->i.getArgument(0));assertEquals("Ali",svc.create(d).getName());
    }

 @Test void all(){
    when(repo.findAll()).thenReturn(List.of());assertTrue(svc.all().isEmpty());
    }
}
